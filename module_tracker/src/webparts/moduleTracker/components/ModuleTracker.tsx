...
const ProgressLabel = ({ label, percent, font, progressBarColor }: { label: string; percent: number; font: string | undefined; progressBarColor: string | undefined }): JSX.Element => (
  <div style={{ marginBottom: 12 }}>
    <label style={{ fontWeight: 600, color: font ?? "#000" }}>{label}</label>
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 4 }}>
      <div style={{ flex: 1, backgroundColor: "#eee", height: 6, borderRadius: 3 }}>
        <div style={{
          width: `${percent}%`,
          height: "100%",
          background: progressBarColor ?? "bf9902",
          borderRadius: 3,
          transition: "width 0.3s ease"
        }} />
      </div>
      <span style={{ fontWeight: 600, color: progressBarColor ?? "#bf9902" }}>{percent}%</span>
    </div>
  </div>
);
  ...
  <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 4 }}>
    <div style={{ flex: 1, backgroundColor: "#eee", height: 6, borderRadius: 3 }}>
      <div style={{
        width: `${percent}%`,
        height: "100%",
        background: font ?? "#bf9902",
        borderRadius: 3,
        transition: "width 0.3s ease"
      }} />
    </div>
    <span style={{ fontWeight: 600, color: font ?? "#bf9902" }}>{percent}%</span>
  </div>
);
...
const ModuleTracker: React.FC<IModuleTrackerProps> = 
  ...
  return (
    <div style={{ borderRadius: "12px", boxShadow: "0px 4px 10px rgba(0,0,0,0.15)", marginBottom: "24px", overflow: "hidden", border: "1px solid #ddd" }}>
      <div onClick={toggleTracker} style={{ background: headerBackground ?? "#000000", color: headerFont ?? "#FFCC00", padding: "16px", cursor: "pointer", fontWeight: 600, display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "1.2rem" }}>
        <span>Module Tracker</span>
        <Icon iconName={trackerOpen ? "ChevronDown" : "ChevronRight"} styles={{ root: { fontSize: 20, color: headerFont ?? "#FFCC00" } }} />
      </div>

      {trackerOpen && (
        <div style={{ background: moduleInternalBackground ?? "#fff", padding: "16px 20px" }}>
      ...
      {trackerOpen && modules.map(mod => (
        <div key={mod.ModuleNumber} style={{ borderBottom: "1px solid #ddd" }}>
          ...
          >
            ...
            <Icon iconName={expandedModules[mod.ModuleNumber] ? "ChevronDown" : "ChevronRight"} styles={{ root: { fontSize: 16, color: moduleHeaderFont ?? "#fff" } }} />
          </div>

          {expandedModules[mod.ModuleNumber] && (
            <div style={{ background: moduleInternalBackground ?? "#fff", padding: "16px 20px" }}>
              <ProgressLabel label={`Module ${mod.ModuleNumber} Video Progress`} percent={mod.VideoProgress} font={moduleInternalFont ?? "#000"} 
              progressBarColor={moduleProgressColor ?? "#bf9902"} />
              <ProgressLabel label={`Module ${mod.ModuleNumber} Quiz Progress`} percent={mod.QuizProgress} font={moduleInternalFont ?? "#000"} 
              progressBarColor={moduleProgressColor ?? "#bf9902"}/>
              {mod.ExamProgress !== undefined && (
                <ProgressLabel label={`Module ${mod.ModuleNumber} Exam Progress`} percent={mod.ExamProgress} font={moduleInternalFont ?? "#000"} 
                progressBarColor={moduleProgressColor ?? "#bf9902"}/>
              )}
            </div>
          )}
