      ...
      {trackerOpen && modules.map(mod => (
        <div key={mod.ModuleNumber} style={{ borderBottom: "1px solid #ddd" }}>
          ...
          {expandedModules[mod.ModuleNumber] && (
            <div style={{ background: moduleInternalBackground, padding: "16px 20px" }}>
              <ProgressLabel label={`Module ${mod.ModuleNumber} Video Progress`} percent={mod.VideoProgress} font={moduleInternalFont} 
              progressBarColor={moduleProgressColor} />
              <ProgressLabel label={`Module ${mod.ModuleNumber} Quiz Progress`} percent={mod.QuizProgress} font={moduleInternalFont} 
              progressBarColor={moduleProgressColor}/>
              {mod.ExamProgress !== undefined && (
                <ProgressLabel label={`Module ${mod.ModuleNumber} Exam Progress`} percent={mod.ExamProgress} font={moduleInternalFont} 
                progressBarColor={moduleProgressColor}/>
              )}
            </div>
          )}
